...
class add_normal_noise():
    ...
    def __init__(self, mu=0., std=1.):
        ...
        self.sigma = torch.tensor(std)

    def __call__(self, tensor):
        #return ( tensor + (torch.randn(tensor.size()).to(self.device) * self.sigma) + self.mu )
        return ( tensor + (torch.randn(tensor.size()) * self.sigma) + self.mu )
...
class roll_channel_axis():
    ...
    def __init__(self, old_pos=0, new_pos=-1):
        #self.device = device
    ...
    def __call__(self, tensor):
        ...
        return torch.tensor(np.rollaxis(tensor.detach().cpu().numpy(), self.old_pos, self.new_pos))
